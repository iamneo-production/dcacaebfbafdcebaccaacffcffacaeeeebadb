...
function UrlValidator(){
    ...
    return(
        <div>
        <div data-testid="message" className={message==='' ?"dnone": isRed ? "cred" : "cgreen"}>{message}</div>
        <form onSubmit={handleSubmit} data-testid = "submit">
            <label className="labelcss">Domain</label>
            <input name="domain" type="text" placeholder="Enter the Domain URL" value={domain} onChange={(event)=>setDomain(event.target.value)} data-testid = "domain"/>
    
            <label className="labelcss">Path</label>
            <input type="text" placeholder="Enter the path variables seperated by comma" value={path} onChange={(event)=>setPath(event.target.value)} data-testid = "path"/>
